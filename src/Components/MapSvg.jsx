...
const MapSvg = () => {
    ...
    useGSAP(() => {
        ...
        const xTo = gsap.quickTo(containerRef.current, "x", { duration: 0.7 })
        const yTo = gsap.quickTo(containerRef.current, "y", { duration: 0.7 })

        gsap.ticker.add(() => {
            const x = gsap.getProperty(circleRef.current, "x")
            const y = gsap.getProperty(circleRef.current, "y")
            xTo(-x)
            yTo(-y)
        })
        ...
        return () => window.removeEventListener("resize", centerContainer)
    }, [])

    return (
        <div className="relative w-full h-[500vh] bg-[#f2efe9]" ref={scrollDistRef}>
            <div
                ref={containerRef}
                className="fixed opacity-0"
                style={{ width: "7900px", height: "5600px" }}
            >
                <img
                ...
                <svg
                    ...
                    className="absolute"
                >
                    <circle ref={circleRef} id="c" r="10" fill="#f00" />
                    <path
                        ref={pathRef}
                        id="p"
                        stroke="#f00"
